...
int main(int argc,char* argv[])
{
    ...
    char ip[20]={0};
    char port[10]={0};
//    char absPath[64]={0};   //修改
    char absPath[1024]={0};
    ServerInit(argv[1],ip,port,absPath);  //运行此项目时,argv[1]传入的是''../conf/server.conf ''
    //创建子线程
    int i,j;
    memset(&f,0,sizeof(f));
    f.pthid=(pthread_t*)calloc(ThreadNum+1,sizeof(pthread_t));  //在内存的动态存储区分配num个长度为size的连续空间,返回一个指向分配起始地址的指针
    ...
    f.state=1;//1为线程池开启
    logfd=open("../log/log",O_WRONLY|O_APPEND|O_CREAT,0666);    //打开log目录下的log文件并设置权限
...
//    char buf[128]={0},bufTim[64]={0};
    char buf[1024]={0},bufTim[512]={0};
    time_t tim;
    int dataLen;
    int currentSize=0;//连上的客户端数目

    pthread_create(&f.pthid[ThreadNum],NULL,supervise,&epfd);  //supervise为监督子线程,超时则关闭

    while(1){
        waitNum=epoll_wait(epfd,evs,MaxNum+1,-1);   //Wait for events on an epoll instance "epfd"
        for(i=0;i<waitNum;i++){
            //如果描述符是socketfd
            if(evs[i].data.fd==socketfd){
                ...
                if(1==dataLen){//1为瞬时命令
                    ...
                    if(tok[buf]&&strcmp(buf,mp[tok[buf]]->pUserInfo->token)==0){
                        ...
                        ret=queInsert(f.threadQue,pnode);  //互斥锁lock、unlock中间区域:临界区代码
...
int userAction(void* p){//登录成功,为用户服务
    ...
    if(strcmp(operate,"cd")==0){   
        ...
        write(logfd,buf,strlen(buf));  //写日志到logfd
        if(strcmp(operand,".")==0){
            ret=1;
            ...
            i=strlen(pnode->pUserInfo->curPath)-1;   //i为当前路径长度
    ...
    /*ls:查看目录下的文件*/
        ...
        strcpy(buf,pnode->pUserInfo->absPath);
        strcat(buf,pnode->pUserInfo->curPath);
        dir=opendir(buf);  //打开一个目录,返回目录指针,在失败的时候返回一个空的指针
        i=strlen(buf)+1;
        buf[i-1]='/';
            //以下是ls的C简单实现:
        while((dnt=readdir(dir))!=NULL){  //本函数用于读取目录,返回的是dirent结构体指针
    ...
    /*remove: 删除文件*/
        ...
        if(1==ret){//文件存在
            ...
            用来将参数file_name所指的文件状态,复制到参数buf所指的结构中
            */
            if(sta.st_nlink>2){  //文件的硬链接数
                remove(buf);
            }
    ...
    while(1){
        ...
        if(strcmp(operate,"puts")==0){
            ...
            if(1==ret){//文件存在,断点续传
                ...
                buf[strlen(buf)]='/';
                strcat(buf,operand);
                stat(buf,&sta);   //用于将buf(所指文件的状态)复制到sta所指的结构中
                send_n(pnode->newfd,&sta.st_size,sizeof(long));
...
int userRegister(int newfd,void* p){
    ...
    if('1'==workNum){//注册第一步,输入用户名
        recv_n(newfd,&dataLen,sizeof(int));
        recv_n(newfd,buf,dataLen);  //buf存储接收到的信息
        sprintf(sql,"select * from UserInfo where UserName='%s'",buf);
        ret=sqlFindData(sql);
        send_n(newfd,&ret,sizeof(int));
        if(ret==0){  //sql中查询不到,则将buf中存储的信息赋给username
...
int userLogin(int newfd,void* p){
    ...
    //客户端登录第二步,输入密码
        ...
        if(strcmp(buf,buf1)==0){//验证成功
            ...
            strcat(pnode->pUserInfo->absPath,pnode->pUserInfo->UserName);  //密码和盐值拼接
    ...
    //第二次登录
        ...
        if(strcmp(buf,buf1)==0){//登录成功
            ...
            send_n(newfd,&dataLen,sizeof(int));
            send_n(newfd,pnode->pUserInfo->token,dataLen);
            tok[pnode->pUserInfo->token]=newfd;    //token与newfd(与客户端通信的sockfd)相对应
            return 0;
        }
        else{
            ret=0;
